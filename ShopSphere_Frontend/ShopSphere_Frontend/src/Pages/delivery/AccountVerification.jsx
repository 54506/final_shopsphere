            ...
            <div className="w-full max-w-lg relative z-10 animate-fadeInUp">
                ...
                <div className="mb-12 text-center">
                    <div className={`inline-flex items-center gap-3 px-5 py-2.5 rounded-2xl mb-8 border transition-all ${isDarkMode ? 'bg-white/5 border-white/10' : 'bg-white border-slate-200 shadow-sm'}`}>
                        ...
                        <span className={`text-[10px] font-black uppercase tracking-[4px] ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>Agent Onboarding Portal</span>
                    </div>
                    ...
                    <p className={`font-bold text-[11px] uppercase tracking-[3px] italic transition-colors ${isDarkMode ? 'text-slate-500' : 'text-slate-400'}`}>Secure verify your logistics endpoint.</p>
                </div>
                ...
                <div className={`backdrop-blur-3xl rounded-[48px] p-10 md:p-14 shadow-2xl relative overflow-hidden border transition-all duration-500 hover:shadow-orange-500/5 ${isDarkMode ? 'bg-white/5 border-white/10' : 'bg-white border-slate-100 shadow-slate-200'}`}>
                    ...
                    <form onSubmit={sendOtp} className="space-y-10">
                        <div className="space-y-4">
                            ...
                            <div className="relative group">
                                ...
                                <input
                                    ...
                                    className={`w-full pl-16 pr-8 py-6 rounded-[28px] border-2 font-black italic outline-none transition-all shadow-inner ${isDarkMode ? 'bg-[#020617] border-transparent focus:border-orange-500 text-white focus:bg-black/40 placeholder:text-slate-600' : 'bg-slate-50 border-slate-100 focus:border-orange-400 text-slate-900 focus:bg-white placeholder:text-slate-400'}`}
                                    required
                                />
                            </div>
                        </div>
                        ...
                        <button
                            type="submit"
                            disabled={loading}
                            className={`w-full py-6 rounded-[32px] text-white text-[11px] font-black uppercase tracking-[5px] shadow-2xl transition-all flex items-center justify-center gap-4 group italic ${loading ? 'opacity-70 cursor-not-allowed' : 'bg-indigo-600 hover:bg-indigo-500 hover:scale-[1.02] active:scale-[0.98] shadow-indigo-900/30'}`}
                        >
                            {loading ? (
                                <>
                ...
                <div className="mt-16 grid grid-cols-2 gap-8 px-6">
                    <div className="flex items-center gap-4 transition-all hover:translate-y-[-2px] group">
                        ...
                        <div className="flex flex-col">
                            <span className={`text-[8px] font-black uppercase tracking-widest ${isDarkMode ? 'text-white/30' : 'text-slate-400'}`}>L_04 Security</span>
                            <span className={`text-[10px] font-black uppercase tracking-wider ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>Encrypted</span>
                        </div>
                    </div>
                    <div className="flex items-center gap-4 text-right justify-end transition-all hover:translate-y-[-2px] group">
                        <div className="flex flex-col">
                            <span className={`text-[8px] font-black uppercase tracking-widest ${isDarkMode ? 'text-white/30' : 'text-slate-400'}`}>Network Sync</span>
                            <span className={`text-[10px] font-black uppercase tracking-wider ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>Enabled</span>
                        </div>
                        <div className={`w-12 h-12 rounded-2xl flex items-center justify-center border transition-all ${isDarkMode ? 'bg-white/5 border-white/10 group-hover:bg-indigo-500/10 group-hover:border-indigo-500/30' : 'bg-white border-slate-100 shadow-sm group-hover:bg-indigo-50 group-hover:border-indigo-200'}`}>
