    ...
    if (loading) {
        return (
            <div className={`flex flex-col items-center justify-center min-h-[60vh] gap-4 transition-colors duration-300 ${isDarkMode ? 'bg-[#0f172a]' : 'bg-[#f8fafc]'}`}>
                ...
                <p className={`text-[10px] font-black uppercase tracking-[3px] ${isDarkMode ? 'text-slate-500' : 'text-slate-400'}`}>Loading history records...</p>
            </div>
        );
    }
            ...
            <div className="max-w-6xl mx-auto px-4 md:px-8">
                {history.length === 0 ? (
                    <div className={`text-center py-24 rounded-[48px] shadow-2xl border-2 border-dashed transition-all ${isDarkMode ? 'bg-white/5 border-white/5' : 'bg-white border-slate-200'}`}>
                        ...
                        <h3 className={`text-2xl font-black uppercase tracking-tight italic ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>No Records Found</h3>
                    ...
                    <div className="grid grid-cols-1 gap-6 md:gap-8">
                        {history.map((order) => (
                            <div
                                key={order.id}
                                className={`rounded-[42px] p-6 md:p-8 shadow-2xl border transition-all duration-500 group relative overflow-hidden ${isDarkMode ? 'bg-white/5 border-white/5 hover:border-indigo-500/20' : 'bg-white border-slate-100'}`}
                            >
                                ...
                                <div className="flex flex-col md:flex-row justify-between items-center gap-8 relative z-10">
                                    <div className="flex-1 w-full text-left">
                                        <div className="flex items-center gap-4 mb-6">
                                            <span className={`px-4 py-1.5 rounded-xl text-[9px] font-black uppercase tracking-[2px] border ${order.status === 'delivered'
                                                ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20'
                                                : 'bg-rose-500/10 text-rose-400 border-rose-500/20'
                                                }`}>
                                                {order.status === 'delivered' ? 'Success' : 'Failed'}
                                            </span>
                                            <span className={`text-[10px] font-black uppercase tracking-[2px] ${isDarkMode ? 'text-slate-600' : 'text-slate-400'}`}>ID #{order.id}</span>
                                        </div>

                                        <h3 className={`text-xl md:text-2xl font-black mb-2 flex items-center gap-4 italic tracking-tight uppercase transition-colors ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>
                                            <div className={`w-10 h-10 rounded-xl flex items-center justify-center group-hover:scale-110 transition-all border shadow-inner ${isDarkMode ? 'bg-[#0f172a] text-indigo-400 border-white/5' : 'bg-slate-50 text-indigo-600 border-slate-200'}`}>
                                                <FaBox size={16} />
                                            </div>
                                            {order.customer_name}
                                        </h3>
                                        ...
                                        <div className="flex flex-wrap items-center gap-6 mt-6">
                                            <div className="flex items-center gap-2 text-gray-500">
                                                ...
                                                <span className="text-[10px] font-black uppercase tracking-widest">{order.delivery_city}</span>
                                            </div>
                                            <div className="flex items-center gap-2 text-gray-500">
                                            ...
                                            <div className="flex items-center gap-2 text-gray-500">
                                                ...
                                                <span className="text-[10px] font-black uppercase tracking-widest">{order.items?.length || 0} Items</span>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className={`flex flex-col md:flex-row items-center gap-8 w-full md:w-auto border-t md:border-t-0 md:border-l pt-8 md:pt-0 md:pl-10 transition-colors ${isDarkMode ? 'border-white/5' : 'border-slate-100'}`}>
                                        <div className="text-center md:text-right min-w-[120px]">
                                            <div className="flex items-baseline justify-center md:justify-end gap-1">
                                                <span className="text-sm font-black text-emerald-500/30 italic font-sans">₹</span>
                                            ...
                                            <p className="text-[9px] font-black text-gray-500 uppercase tracking-[3px] mt-1 italic">Earnings</p>
                                        </div>

                                        <button
                                            onClick={() => navigate(`/delivery/order/${order.id}`)}
                                            className={`w-full md:w-auto px-10 py-4 border rounded-[24px] font-black uppercase tracking-[3px] text-[10px] transition-all active:scale-95 italic ${isDarkMode ? 'bg-white/5 border-white/5 text-slate-400 hover:bg-indigo-600 hover:text-white' : 'bg-slate-50 border-slate-200 text-slate-500 hover:bg-slate-900 hover:text-white'}`}
                                        >
                                            View Order
                                        </button>
                                    </div>
