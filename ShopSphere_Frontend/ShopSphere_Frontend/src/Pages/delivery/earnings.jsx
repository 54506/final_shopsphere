        ...
        <div className={`w-full min-h-screen pb-20 transition-colors duration-300 ${isDarkMode ? 'bg-[#0f172a]' : 'bg-[#f8fafc]'}`}>
                    ...
                    <div className={`flex p-1.5 rounded-2xl border self-center gap-1 transition-all ${isDarkMode ? 'bg-white/5 border-white/5' : 'bg-slate-100 border-slate-200'}`}>
                        {['today', 'monthly', 'yearly'].map((f) => (
                            <button
                                key={f}
                                onClick={() => setFilter(f)}
                                className={`px-5 md:px-8 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-[2px] transition-all duration-300 capitalize ${filter === f ? 'bg-indigo-600 text-white shadow-lg scale-105' : isDarkMode ? 'text-slate-500 hover:text-white' : 'text-slate-400 hover:text-slate-900'}`}
                            >
                                {f}
                            </button>
                        ))}
            ...
            <div className="max-w-7xl mx-auto px-4 md:px-8 grid grid-cols-1 lg:grid-cols-12 gap-8 md:gap-10">
                        ...
                        <div className="relative z-10">
                            <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-8 md:mb-12 gap-4">
                                <div className="flex items-center gap-4">
                                    ...
                                    <h2 className={`text-xl md:text-2xl font-black tracking-tight italic transition-colors ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>Payment Records</h2>
                                </div>
                                <div className={`text-[9px] font-black uppercase tracking-widest px-4 py-2 rounded-full border transition-all ${isDarkMode ? 'text-slate-500 bg-white/5 border-white/5' : 'text-slate-400 bg-slate-50 border-slate-200'}`}>
                            ...
                            {txLoading ? (
                                <div className="flex flex-col items-center justify-center py-24 gap-4 text-center">
                                    ...
                                    <p className={`text-[10px] font-black uppercase tracking-[3px] ${isDarkMode ? 'text-slate-500' : 'text-slate-400'}`}>Loading records...</p>
                                </div>
                                ...
                                <div className={`text-center py-24 rounded-[32px] border-2 border-dashed transition-all ${isDarkMode ? 'bg-[#0f172a]/50 border-white/5' : 'bg-slate-50/50 border-slate-100'}`}>
                                    ...
                                    <p className={`text-lg font-black uppercase tracking-widest transition-colors ${isDarkMode ? 'text-slate-600' : 'text-slate-300'}`}>No Records Yet</p>
                                    <p className="text-sm text-gray-500 mt-2 font-medium italic">Complete deliveries to see your earnings here.</p>
                                </div>
                            ) : (
                                <div className="overflow-x-auto no-scrollbar">
                                    <table className="w-full text-left min-w-[560px]">
                                        <thead className={`text-[9px] font-black uppercase tracking-[3px] border-b transition-colors ${isDarkMode ? 'text-slate-600 border-white/5' : 'text-slate-400 border-slate-100'}`}>
                                            <tr>
                                            ...
                                            {commissions.map((c, idx) => (
                                                <tr key={c.id || idx} className={`group transition-all duration-300 ${isDarkMode ? 'hover:bg-white/5' : 'hover:bg-slate-50'}`}>
                                                    <td className="py-5 px-4 text-left">
                                                        <div className="flex items-center gap-4">
                                                            <div className={`w-9 h-9 rounded-2xl border flex items-center justify-center transition-all ${isDarkMode ? 'bg-white/5 border-white/5 group-hover:bg-indigo-600/10 group-hover:border-indigo-500/20' : 'bg-slate-50 border-slate-100 group-hover:bg-white group-hover:border-indigo-200'}`}>
                                                                <FaMotorcycle className={`${isDarkMode ? 'text-indigo-400' : 'text-indigo-600'} text-sm`} />
                                                            </div>
                                                            <div>
                                                                <p className={`font-black text-sm tracking-tight italic transition-colors ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>
                                                                    {c.delivery_assignment?.order?.order_number || c.notes || `TRX-${c.id}`}
                                                                </p>
                                                                <p className={`text-[9px] font-black uppercase tracking-widest mt-0.5 transition-colors ${isDarkMode ? 'text-slate-500' : 'text-slate-400'}`}>
                                                                    {c.created_at ? new Date(c.created_at).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' }) : '––'}
                                                                </p>
                                                            </div>
                                                        </div>
                                                    </td>
                                                    <td className={`py-5 px-4 text-right font-black text-sm transition-colors ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>
                                                        ₹{parseFloat(c.base_fee || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}
                                                    </td>
                                                    <td className="py-5 px-4 text-right font-black text-indigo-400 text-sm">
                                                        +₹{parseFloat(c.distance_bonus || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}
                                                    </td>
                                                    <td className="py-5 px-4 text-right">
                                                        <span className="font-black text-lg text-emerald-400 tracking-tighter italic">
                                                            ₹{parseFloat(c.total_commission || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}
                                                        </span>
                                                    </td>
                                                    <td className="py-5 px-4 text-center">
                                                        <span className={`text-[9px] font-black uppercase tracking-[2px] px-3 py-1.5 rounded-xl border ${c.status === 'paid' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' :
                                                            c.status === 'approved' ? 'bg-indigo-500/10 text-indigo-400 border-indigo-500/20' :
                                        ...
                                        <tfoot className={`border-t-2 transition-colors ${isDarkMode ? 'border-white/5' : 'border-slate-100'}`}>
                                            <tr>
                                                <td className={`py-6 px-4 font-black uppercase tracking-widest italic text-[10px] text-left transition-colors ${isDarkMode ? 'text-slate-500' : 'text-slate-400'}`}>Total</td>
                                                <td className={`py-6 px-4 text-right font-black text-sm transition-colors ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>
                                                    ₹{commissions.reduce((s, c) => s + parseFloat(c.base_fee || 0), 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}
                                                </td>
                                                <td className="py-6 px-4 text-right font-black text-indigo-400 text-sm">
                                                    ₹{commissions.reduce((s, c) => s + parseFloat(c.distance_bonus || 0), 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}
                                                </td>
                                                <td className="py-6 px-4 text-right">
                                                    <span className="font-black text-2xl text-emerald-400 tracking-tighter italic">
                ...
                <div className="lg:col-span-4 space-y-8 md:space-y-10">
                    {/* Wallet Card */}
                    <div className={`rounded-[40px] md:rounded-[48px] p-8 md:p-10 text-white relative overflow-hidden shadow-2xl group transition-all ${isDarkMode ? 'bg-indigo-600 shadow-indigo-900/40' : 'bg-slate-900 shadow-slate-200'}`}>
                        ...
                        <div className="relative z-10 text-left">
                            <div className="flex items-center gap-4 mb-10">
                                ...
                                <div>
                                    <h3 className="text-lg font-black tracking-tight italic uppercase">My Wallet</h3>
                                    <p className="text-[10px] font-black text-indigo-200 uppercase tracking-[3px] mt-0.5 italic">Available Balance</p>
                                </div>
                            </div>

                            <div className="space-y-1 mb-8">
                                <span className="text-[10px] font-black uppercase tracking-[4px] opacity-60">Total Earned</span>
                                <div className="flex items-baseline gap-2">
                                    <span className="text-2xl font-black text-white/60 italic">₹</span>
                                    <h2 className="text-5xl font-black tracking-tighter italic">{totalEarned.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</h2>
                                </div>
                            </div>

                            <div className="grid grid-cols-2 gap-4 mb-10">
                                <div className="bg-white/10 border border-white/20 rounded-2xl p-4 backdrop-blur-sm text-left">
                                    <p className="text-[9px] font-black opacity-50 uppercase tracking-[2px] mb-1.5">Paid Out</p>
                                    <p className="text-lg font-black text-emerald-300 italic">₹{paidAmount.toLocaleString('en-IN', { minimumFractionDigits: 0 })}</p>
                                </div>
                                <div className="bg-white/10 border border-white/20 rounded-2xl p-4 backdrop-blur-sm text-left">
                                    <p className="text-[9px] font-black opacity-50 uppercase tracking-[2px] mb-1.5">On Hold</p>
                                    <p className="text-lg font-black text-amber-300 italic">₹{(pendingAmount + approvedAmount).toLocaleString('en-IN', { minimumFractionDigits: 0 })}</p>
                                </div>
                            </div>

                            <div className="space-y-4">
                                <div className="relative">
                                    <div className="absolute left-5 top-1/2 -translate-y-1/2 font-black text-white/40 text-lg italic">₹</div>
                                    <input
                                        type="number"
                                        placeholder="Enter amount to withdraw"
                                        value={withdrawAmount}
                                        onChange={(e) => setWithdrawAmount(e.target.value)}
                                        className="w-full bg-white/10 border border-white/20 rounded-[20px] py-4 pl-10 pr-5 text-white font-black text-lg outline-none focus:border-white/50 transition-all placeholder:text-white/20 shadow-inner appearance-none italic"
                                    />
                                </div>
                                <button
                                    onClick={handleWithdraw}
                                    className={`w-full flex items-center justify-center gap-4 py-5 rounded-[20px] font-black uppercase tracking-[4px] text-[11px] transition-all shadow-2xl active:scale-95 italic ${isDarkMode ? 'bg-white text-indigo-700 hover:bg-slate-50' : 'bg-white text-slate-900 hover:bg-slate-50'}`}
                                >
                                    <FaArrowUp />
                                    Withdraw Money
                                </button>
                    ...
                    <div className={`rounded-[40px] md:rounded-[48px] p-8 md:p-10 border shadow-2xl relative overflow-hidden transition-all ${isDarkMode ? 'bg-white/5 border-white/5' : 'bg-white border-slate-100'}`}>
                        <div className="flex items-center gap-4 mb-8">
                            ...
                            <h3 className={`text-lg md:text-xl font-black tracking-tight italic uppercase text-left transition-colors ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>Payment Rules</h3>
                        </div>
                        ...
                        <div className="space-y-6">
                            <div className="space-y-5 text-left">
                                {[
                                    ...
                                    <div key={i} className="flex items-start gap-4">
                                        ...
                                        <div>
                                            <p className={`text-[11px] font-black uppercase tracking-widest transition-colors ${isDarkMode ? 'text-white' : 'text-slate-800'}`}>{item.label}</p>
                                            <p className="text-[10px] text-gray-500 mt-1 font-medium italic">{item.desc}</p>
                                        </div>
                                    </div>
                                ))}
                            </div>
                            ...
                            <div className={`pt-6 border-t space-y-4 text-left transition-colors ${isDarkMode ? 'border-white/5' : 'border-slate-100'}`}>
                                <h4 className={`text-[9px] font-black uppercase tracking-[4px] italic transition-colors ${isDarkMode ? 'text-slate-600' : 'text-slate-400'}`}>Withdrawal Rules</h4>
                                <ul className="space-y-3">
                                    {[
                                        ...
                                        <li key={i} className="flex items-center gap-3 text-[11px] font-black text-gray-500 italic">
