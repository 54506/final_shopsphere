...
export default function DeliveryIdentityDetails() {
    ...
    return (
        <div className={`min-h-screen flex items-center justify-center p-6 relative overflow-hidden transition-colors duration-500 ${isDarkMode ? 'bg-[#0f172a]' : 'bg-[#f8fafc]'}`}>
            ...
            <div className={`absolute top-0 right-0 w-[800px] h-[800px] rounded-full translate-x-1/2 -translate-y-1/2 blur-[150px] pointer-events-none transition-opacity duration-1000 ${isDarkMode ? 'bg-indigo-500/10 opacity-100' : 'bg-indigo-500/5 opacity-50'}`}></div>
            ...
            <div className="w-full max-w-2xl relative z-10 animate-fadeInUp">
                <div className="mb-12 text-center">
                    <div className={`inline-flex items-center gap-3 px-5 py-2.5 rounded-2xl mb-8 border transition-all ${isDarkMode ? 'bg-white/5 border-white/10' : 'bg-white border-slate-200 shadow-sm'}`}>
                        ...
                        <span className={`text-[10px] font-black uppercase tracking-[4px] ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>Agent Dossier Setup</span>
                    </div>
                    <h1 className={`text-5xl lg:text-6xl font-black tracking-tighter italic uppercase mb-4 transition-colors ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>
                        Profile <br />
                        <span className="text-transparent bg-clip-text bg-gradient-to-r from-orange-400 via-indigo-400 to-purple-500">Identity.</span>
                    </h1>
                    <p className={`font-bold text-[11px] uppercase tracking-[3px] italic transition-colors ${isDarkMode ? 'text-slate-500' : 'text-slate-400'}`}>Step 2: Construct your operational profile</p>
                </div>
