            ...
            <div className={`flex h-screen items-center justify-center transition-colors duration-300 ${isDarkMode ? 'bg-[#0f172a]' : 'bg-[#F8FAFC]'}`}>
                <div className="flex flex-col items-center gap-4">
                    <div className="relative">
                        ...
                        <Truck className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-6 h-6 text-indigo-500" />
                    </div>
                    <p className={`text-[10px] font-black uppercase tracking-[0.3em] ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>Synchronizing Fleet Telemetry</p>
                </div>
            </div>
        );
    }
        ...
        return (
            <div className={`flex h-screen items-center justify-center transition-colors duration-300 ${isDarkMode ? 'bg-[#0f172a]' : 'bg-[#F8FAFC]'}`}>
                <div className="text-center max-w-sm px-6">
                    ...
                    <h2 className={`text-2xl font-black mb-2 ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>Node Disconnected</h2>
                    ...
                    <button onClick={() => navigate(-1)} className="px-8 py-3 bg-indigo-600 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-indigo-500 transition-all shadow-lg shadow-indigo-600/20">Return to Registry</button>
                </div>
            </div>
        );
    }
            ...
            <div className="flex-1 flex flex-col min-w-0 relative">
                <header className={`border-b px-8 h-20 flex items-center justify-between sticky top-0 z-20 transition-colors duration-300 ${isDarkMode ? 'bg-[#0f172a]/80 border-slate-800 backdrop-blur-md' : 'bg-white border-slate-100 shadow-sm'}`}>
                    <div className="flex items-center gap-4">
                        <button onClick={() => navigate(-1)} className={`p-2 rounded-xl border transition-all ${isDarkMode ? 'bg-slate-900 border-slate-700 text-slate-400 hover:text-white' : 'bg-white border-slate-200 text-slate-400 hover:text-indigo-600 shadow-sm'}`}>
                            <ArrowLeft className="w-5 h-5" />
                        </button>
                        <div>
                            <h1 className={`text-lg font-bold tracking-tight ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>Entity Dossier</h1>
                            <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Security Clearance Review</p>
                        </div>
                    </div>
                    <div className="flex items-center gap-6">
                        <NotificationBell />
                        <div className={`hidden lg:flex items-center border rounded-lg px-3 py-1.5 text-[10px] font-bold uppercase tracking-widest gap-2 ${isDarkMode ? 'bg-amber-500/10 border-amber-500/20 text-amber-500' : 'bg-amber-50 border-amber-100 text-amber-600'}`}>
                ...
                <main className="flex-1 overflow-y-auto p-4 md:p-8">
                    <div className="max-w-5xl mx-auto space-y-8 pb-32">
                        ...
                        <motion.div
                            ...
                            className={`rounded-[3rem] p-8 md:p-12 relative overflow-hidden border transition-all duration-300 ${isDarkMode ? 'bg-[#1e293b]/50 border-slate-800 shadow-2xl shadow-indigo-500/5' : 'bg-white border-slate-100 shadow-sm'}`}
                        >
                            <div className="relative z-10 flex flex-col md:flex-row gap-8 items-center md:items-start text-center md:text-left">
                                <div className="relative group">
                                    <div className="absolute -inset-1 bg-gradient-to-r from-indigo-500 to-purple-500 rounded-[2.5rem] blur opacity-25 group-hover:opacity-50 transition duration-1000 group-hover:duration-200" />
                                    <div className={`relative w-32 h-32 rounded-[2.5rem] flex items-center justify-center text-5xl font-black transition-all ${isDarkMode ? 'bg-slate-900 text-white border border-slate-700' : 'bg-indigo-600 text-white shadow-xl shadow-indigo-600/20'}`}>
                                ...
                                <div className="flex-1">
                                    <div className="flex flex-wrap items-center justify-center md:justify-start gap-3 mb-4">
                                        <span className={`px-4 py-1 rounded-full text-[10px] font-black uppercase tracking-widest border ${agent.is_blocked ? (isDarkMode ? 'bg-rose-500/10 text-rose-400 border-rose-500/20' : 'bg-rose-50 text-rose-600 border-rose-100') :
                                        ...
                                        <span className={`px-4 py-1 rounded-full text-[10px] font-black uppercase tracking-widest border ${isDarkMode ? 'bg-slate-800 border-slate-700 text-slate-500' : 'bg-slate-50 text-slate-400 border-slate-100'}`}>
                                            PROTOCOL ID: {agent.id}
                                        </span>
                                    </div>
                                    <h2 className={`text-4xl font-black tracking-tight mb-2 ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>{agent.user_username || 'Logistics Operational Agent'}</h2>
                                    <div className="flex flex-wrap items-center justify-center md:justify-start gap-6">
                                        <div className="flex items-center gap-2 text-slate-500 text-sm font-medium">
                                            <Mail className="w-4 h-4 text-indigo-500" /> {agent.user_email}
                                        </div>
                                        <div className="flex items-center gap-2 text-slate-500 text-sm font-medium">
                                            <Phone className="w-4 h-4 text-indigo-500" /> {agent.phone_number}
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            ...
                            <section className={`rounded-[2.5rem] p-8 border transition-all duration-300 ${isDarkMode ? 'bg-[#1e293b]/50 border-slate-800' : 'bg-white border-slate-100 shadow-sm'}`}>
                                <h3 className="text-xs font-black text-slate-500 uppercase tracking-[0.2em] mb-8 flex items-center gap-2">
                                    <Fingerprint className="w-4 h-4 text-indigo-500" /> Identity Matrix
                                </h3>
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-8">
                                    {[
                                        ...
                                        <div key={i} className="space-y-1">
                                            <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-1.5"><item.icon className="w-3 h-3" /> {item.label}</p>
                                ...
                                <div className={`p-6 rounded-3xl border ${isDarkMode ? 'bg-slate-900/50 border-slate-800' : 'bg-slate-50 border-transparent shadow-inner'}`}>
                                    <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-2 flex items-center gap-1.5"><MapPinned className="w-3 h-3" /> Registry Address</p>
                            ...
                            <section className={`rounded-[2.5rem] p-8 border transition-all duration-300 ${isDarkMode ? 'bg-[#1e293b]/50 border-slate-800' : 'bg-white border-slate-100 shadow-sm'}`}>
                                ...
                                <div className="space-y-6">
                                    <div className={`p-8 rounded-3xl border flex flex-col items-center justify-center gap-4 relative overflow-hidden group ${isDarkMode ? 'bg-slate-900/50 border-slate-800 text-indigo-400' : 'bg-indigo-50 border-transparent text-indigo-600 shadow-inner'}`}>
                                        <div className="absolute top-0 right-0 p-4 opacity-5 group-hover:scale-110 transition-transform">
                                            <Truck className="w-24 h-24" />
                                        </div>
                                        <Truck className="w-12 h-12 relative z-10" />
                                        <div className="text-center relative z-10">
                                            <p className="text-[10px] font-black uppercase tracking-widest opacity-60 mb-1">Unit Model / Type</p>
                                            <p className={`text-2xl font-black uppercase ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>{agent.vehicle_type}</p>
                                        </div>
                                    </div>
                                    <div className="grid grid-cols-2 gap-4">
                                        <div className={`p-4 rounded-2xl border ${isDarkMode ? 'bg-slate-900/50 border-slate-800' : 'bg-slate-50 border-transparent'}`}>
                                            <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1">Plate Registry</p>
                                            <p className={`text-sm font-black font-mono tracking-tighter ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>{agent.vehicle_number}</p>
                                        </div>
                                        <div className={`p-4 rounded-2xl border ${isDarkMode ? 'bg-slate-900/50 border-slate-800' : 'bg-slate-50 border-transparent'}`}>
                                            <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1">Capacity Index</p>
                                            <p className={`text-sm font-black ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>{agent.vehicle_type === 'Truck' ? 'HIGH LOAD' : 'STANDARD'}</p>
                                        </div>
                                    </div>
                                </div>
                            </section>
                                    ...
                                    {[
                                        ...
                                        <div key={i} className={`p-6 rounded-3xl border transition-all hover:translate-y-[-2px] ${isDarkMode ? 'bg-slate-900/50 border-slate-800 hover:border-emerald-500/50' : 'bg-slate-50 border-transparent hover:border-emerald-200 shadow-sm'}`}>
                                            <div className="flex items-center gap-3 mb-3 text-emerald-500">
                                                ...
                                                <p className="text-[9px] font-black uppercase tracking-widest opacity-60">{bank.label}</p>
                                            </div>
                            ...
                            <section className={`rounded-[2.5rem] p-8 border transition-all duration-300 md:col-span-2 ${isDarkMode ? 'bg-[#1e293b]/50 border-slate-800' : 'bg-white border-slate-100 shadow-sm'}`}>
                                <h3 className="text-xs font-black text-slate-500 uppercase tracking-[0.2em] mb-8 flex items-center gap-2">
                                    <FileText className="w-4 h-4 text-indigo-500" /> Compliance Artifacts
                                </h3>
                                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                                    {[
                                        ...
                                        { title: 'Identity Matrix', type: agent.id_type, detail: agent.id_number, file: agent.id_proof_file, iconColor: 'text-indigo-500' },
                                        ...
                                        doc.file ? (
                                            <div key={idx}
                                                ...
                                                className={`p-5 rounded-3xl border group cursor-pointer transition-all hover:translate-y-[-4px] ${isDarkMode ? 'bg-slate-900/50 border-slate-800 hover:border-indigo-500/50 hover:bg-indigo-500/5 shadow-2xl shadow-indigo-500/5' : 'bg-slate-50 border-transparent hover:border-indigo-200 shadow-inner hover:shadow-lg hover:shadow-indigo-100'}`}
                                            >
                                                <div className="flex items-center gap-4 mb-4">
                                                    ...
                                                    <div className="flex-1 min-w-0">
                                                        <p className={`text-xs font-black truncate ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>{doc.title}</p>
                                                        <p className="text-[9px] font-black text-slate-500 uppercase tracking-tighter opacity-70">{doc.type}</p>
                                                    </div>
                                                </div>
                                                <div className={`pt-4 border-t transition-colors ${isDarkMode ? 'border-slate-800' : 'border-indigo-100'}`}>
                                                    <div className="flex items-center justify-between">
                                                        <p className="text-[10px] font-mono font-bold text-slate-500 truncate">{doc.detail || 'SECURE SCAN'}</p>
                                                        <ArrowUpRight className="w-3.5 h-3.5 text-indigo-500 opacity-0 group-hover:opacity-100 transition-all" />
                                                    </div>
                                                    {doc.expiry && (
                                                        <p className="text-[9px] font-black text-rose-500 mt-2 uppercase tracking-widest flex items-center gap-1">
                ...
                <div className={`fixed bottom-0 right-0 left-0 border-t backdrop-blur-xl transition-all duration-300 py-6 px-10 z-[100] ${isDarkMode ? 'bg-[#0f172a]/90 border-slate-800 shadow-2xl shadow-black/50' : 'bg-white/90 border-slate-100 shadow-2xl shadow-slate-200'}`} style={{ marginLeft: isSidebarOpen ? '280px' : '8rem' }}>
                    <div className="max-w-5xl mx-auto flex flex-col md:flex-row items-center justify-between gap-6">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest leading-none mb-1">Fleet Security Node {agent.id}</p>
                                <h4 className={`text-sm font-black ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>Decision Required: {agent.user_username}</h4>
                            </div>
                        </div>
                        <div className="flex items-center gap-3 w-full md:w-auto">
                            {canReject && (
                                <button
                                    ...
                                    className={`flex-1 md:flex-none px-8 py-3.5 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${isDarkMode ? 'bg-slate-800 text-slate-400 hover:bg-rose-500/10 hover:text-rose-400' : 'bg-slate-100 text-slate-500 hover:bg-rose-50 hover:text-rose-600'
                                        }`}
                                >
                                    Reject Protocol
                                </button>
                            ...
                            {canBlock && (
                                <button
                                    ...
                                    disabled={isActioning}
                                    className={`flex-1 md:flex-none px-8 py-3.5 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${isDarkMode ? 'bg-rose-500/10 text-rose-500 hover:bg-rose-500 border border-rose-500/20 hover:text-white' : 'bg-rose-50 text-rose-600 hover:bg-rose-600 hover:text-white border border-rose-100'
                                        }`}
                                >
                                    Revoke Access
                                </button>
                            ...
                            {canUnblock && (
                                <button
                                    ...
                                    disabled={isActioning}
                                    className="flex-1 md:flex-none px-8 py-3.5 bg-emerald-600 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl shadow-emerald-500/20 hover:bg-emerald-500 hover:translate-y-[-2px] active:translate-y-[1px] transition-all"
                                >
                                    Restore Protocol
                                </button>
                            )}
                            {canApprove && (
                                <button
                                    ...
                                    disabled={isActioning}
                                    className="flex-1 md:flex-none px-12 py-3.5 bg-indigo-600 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl shadow-indigo-600/20 hover:bg-indigo-500 hover:translate-y-[-2px] active:translate-y-[1px] transition-all flex items-center justify-center gap-2"
                                >
