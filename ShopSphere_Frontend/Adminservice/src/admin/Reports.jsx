...
const StatCard = ({ label, value, icon: Icon, color, prefix = '', sub = '', isDark }) => {
    const colorMap = {
        indigo: { bg: isDark ? 'bg-indigo-500/10' : 'bg-indigo-50', text: 'text-indigo-400' },
        ...
        violet: { bg: isDark ? 'bg-violet-500/10' : 'bg-violet-50', text: 'text-violet-400' },
    ...
    return (
        <div className={`p-6 rounded-[2rem] border relative overflow-hidden group transition-colors
            ...
            <div className={`text-xs font-black uppercase tracking-widest mb-1 ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>{label}</div>
...
const StatusBadge = ({ status, isDark }) => {
    const map = {
        ...
        processing: isDark ? 'bg-violet-500/10 text-violet-400 border-violet-500/20' : 'bg-violet-50 text-violet-700 border-violet-100',
    };
    ...
    return <span className={`px-2.5 py-1 rounded-full text-[10px] font-black uppercase border tracking-tight ${cls}`}>{status}</span>;
};
...
const ProgressRow = ({ label, count, max, color = '#6366f1', isDark }) => (
    <div className={`flex items-center gap-3 py-2.5 border-b last:border-0 ${isDark ? 'border-slate-700/50' : 'border-slate-50'}`}>
        ...
        <div className={`w-10 text-right text-sm font-black ${isDark ? 'text-slate-300' : 'text-slate-800'}`}>{count}</div>
    </div>
);
...
const PanelTitle = ({ children, icon: Icon, iconClass = 'text-indigo-500', isDark }) => (
    <h3 className={`text-sm font-black mb-5 flex items-center gap-2 uppercase tracking-tight ${isDark ? 'text-white' : 'text-slate-900'}`}>
...
const SalesReport = ({ data, isDark }) => (
    <div className="space-y-8">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            <StatCard label="Total Orders" value={data.total_orders} icon={ShoppingCart} color="indigo" isDark={isDark} />
            <StatCard label="Total Revenue" value={data.total_revenue} icon={TrendingUp} color="emerald" prefix="₹" isDark={isDark} />
            <StatCard label="Avg Order Value" value={data.avg_order_value} icon={IndianRupee} color="violet" prefix="₹" isDark={isDark} />
            ...
            <Panel isDark={isDark}>
                ...
                {[
                    ...
                    <div key={label} className={`flex justify-between items-center py-2.5 border-b last:border-0 ${isDark ? 'border-slate-700/50' : 'border-slate-50'}`}>
                        ...
                        <span className="text-sm font-black text-emerald-500">{rupee(value)}</span>
                    </div>
                ))}
            </Panel>
                ...
                <div className="overflow-y-auto max-h-56">
                    <table className="w-full text-left">
                        <thead className={`sticky top-0 border-b ${isDark ? 'bg-[#1e293b] border-slate-700/50' : 'bg-white border-slate-100'}`}>
                            <tr>
                                <th className={`pb-3 text-[10px] font-black uppercase tracking-widest ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>Date</th>
                                <th className={`pb-3 text-[10px] font-black uppercase tracking-widest text-center ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>Orders</th>
                                <th className={`pb-3 text-[10px] font-black uppercase tracking-widest text-right ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>Revenue</th>
                            </tr>
                        </thead>
                            ...
                            {(data.daily_revenue ?? []).length === 0 ? (
                                ...
                                (data.daily_revenue ?? []).map((d, i) => (
                                    <tr key={i} className={`transition-colors ${isDark ? 'hover:bg-slate-700/30' : 'hover:bg-slate-50/50'}`}>
                                        ...
                                        <td className={`py-2.5 text-sm font-black text-center ${isDark ? 'text-slate-200' : 'text-slate-900'}`}>{d.orders}</td>
                                        <td className="py-2.5 text-sm font-black text-emerald-500 text-right">{rupee(d.revenue)}</td>
                                    </tr>
                                ))
                            )}
                        </tbody>
...
const CommissionReport = ({ data, isDark }) => (
    <div className="space-y-8">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <StatCard label="Gross Sales" value={data.total_gross} icon={TrendingUp} color="indigo" prefix="₹" isDark={isDark} />
                    ...
                    <thead className={`border-b ${isDark ? 'bg-slate-800/50 border-slate-700/50' : 'bg-slate-50/70 border-slate-100'}`}>
                        <tr>
                            {['#', 'Vendor', 'Gross (₹)', 'Commission (₹)', 'Net (₹)', 'Orders'].map((h, i) => (
                                <th key={h} className={`px-4 py-4 text-[10px] font-black uppercase tracking-widest ${isDark ? 'text-slate-500' : 'text-slate-400'} ${i >= 2 ? 'text-right' : ''} ${i === 5 ? 'text-center' : ''}`}>{h}</th>
                            ))}
                        </tr>
                    </thead>
                    <tbody className={`divide-y ${isDark ? 'divide-slate-700/50' : 'divide-slate-50'}`}>
                                ...
                                <tr key={i} className={`transition-colors ${isDark ? 'hover:bg-slate-700/30' : 'hover:bg-slate-50/50'}`}>
                                    <td className="px-4 py-3.5">
                                        <span className={`inline-flex items-center justify-center w-7 h-7 rounded-full text-[10px] font-black
                                            ${i === 0 ? 'bg-amber-100 text-amber-700' : i === 1 ? (isDark ? 'bg-slate-700 text-slate-300' : 'bg-slate-200 text-slate-600') : i === 2 ? 'bg-orange-100 text-orange-700' : (isDark ? 'bg-slate-800 text-slate-500' : 'bg-slate-50 text-slate-400')}`}>
                                            {i + 1}
                                        </span>
                                    </td>
                                    ...
                                    <td className="px-4 py-3.5 text-sm font-black text-emerald-500 text-right">{rupee(v.total_net)}</td>
                                    <td className={`px-4 py-3.5 text-sm font-black text-center ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>{v.order_count}</td>
                                </tr>
                            ))
                        )}
                    </tbody>
...
const VendorReport = ({ data, isDark }) => (
    <div className="space-y-8">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            <StatCard label="Total Vendors" value={data.total_vendors} icon={Store} color="indigo" isDark={isDark} />
                    ...
                    <thead className={`border-b ${isDark ? 'bg-slate-800/50 border-slate-700/50' : 'bg-slate-50/70 border-slate-100'}`}>
                        <tr>
                            {['#', 'Shop Name', 'Net Earnings (₹)', 'Orders'].map((h, i) => (
                                <th key={h} className={`px-4 py-4 text-[10px] font-black uppercase tracking-widest ${isDark ? 'text-slate-500' : 'text-slate-400'} ${i === 2 ? 'text-right' : ''} ${i === 3 ? 'text-center' : ''}`}>{h}</th>
                            ))}
                        </tr>
                    </thead>
                    <tbody className={`divide-y ${isDark ? 'divide-slate-700/50' : 'divide-slate-50'}`}>
    ...
    return (
        <div className="space-y-8">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                ...
                <StatCard label="Orders (30 days)" value={data.orders_this_month} icon={Activity} color="violet" isDark={isDark} />
            </div>
                        ...
                        <thead className={`border-b ${isDark ? 'bg-slate-800/50 border-slate-700/50' : 'bg-slate-50/70 border-slate-100'}`}>
                            <tr>
                                {['#', 'Product', 'Qty Sold', 'Revenue (₹)'].map((h, i) => (
                                    <th key={h} className={`px-4 py-4 text-[10px] font-black uppercase tracking-widest ${isDark ? 'text-slate-500' : 'text-slate-400'} ${i === 2 ? 'text-center' : ''} ${i === 3 ? 'text-right' : ''}`}>{h}</th>
                                ))}
                            </tr>
                        </thead>
                        <tbody className={`divide-y ${isDark ? 'divide-slate-700/50' : 'divide-slate-50'}`}>
                            {(data.top_products ?? []).length === 0 ? (
                                ...
                                (data.top_products ?? []).map((p, i) => (
                                    <tr key={i} className={`transition-colors ${isDark ? 'hover:bg-slate-700/30' : 'hover:bg-slate-50/50'}`}>
                                        ...
                                        <td className={`px-4 py-3 text-sm font-black text-center ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>{p.total_qty}</td>
                                        <td className="px-4 py-3 text-sm font-black text-emerald-500 text-right">{rupee(p.total_revenue)}</td>
                                    </tr>
                                ))
                            )}
                        </tbody>
...
const DeliveryReport = ({ data, isDark }) => (
    <div className="space-y-8">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            <StatCard label="Total Agents" value={data.total_agents} icon={Users} color="indigo" isDark={isDark} />
                ...
                {(() => {
                    ...
                    return (
                        <div className="text-center py-4">
                            <div className={`text-6xl font-black mb-2 ${isDark ? 'text-white' : 'text-slate-900'}`}>
                                {rate !== null ? `${rate}%` : '—'}
                            </div>
                            <div className={`text-xs uppercase tracking-widest font-black ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <div className="flex items-center gap-2">
                                <h1 className={`text-xl font-bold tracking-tight ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>Reports</h1>
                                <span className="bg-emerald-500/10 text-emerald-400 text-[10px] font-black px-2 py-0.5 rounded-full uppercase border border-emerald-500/20 tracking-widest leading-none">Live Data</span>
                            </div>
                            {lastRefreshed && (
                    ...
                    {isLoading && !reportData ? (
                        <div className="flex flex-col items-center justify-center min-h-[400px] gap-4">
                            <motion.div animate={{ rotate: 360 }} transition={{ duration: 1, repeat: Infinity, ease: 'linear' }}>
                                <RefreshCcw className={`w-10 h-10 ${isDarkMode ? 'text-indigo-400' : 'text-indigo-300'}`} />
                            </motion.div>
                            <p className={`text-[10px] font-black uppercase tracking-[0.4em] ${isDarkMode ? 'text-slate-600' : 'text-slate-300'}`}>Loading Report Data...</p>
                        </div>
                    ) : error ? (
                        <div className="flex flex-col items-center justify-center min-h-[400px] gap-4">
                            ...
                            <div className="text-center">
                                <p className={`text-sm font-black mb-1 ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>Failed to Load Reports</p>
                                <p className={`text-xs max-w-sm ${isDarkMode ? 'text-slate-500' : 'text-slate-400'}`}>{error}</p>
                            </div>
                            <button onClick={loadReports} className="px-6 py-2.5 bg-indigo-600 text-white text-xs font-bold rounded-xl hover:bg-indigo-700 transition-all">
                                Try Again
                            </button>
                        </div>
                    ) : (
