            ...
            <div className="flex-1 flex flex-col min-w-0">
                <header className={`border-b px-8 h-20 flex items-center justify-between sticky top-0 z-20 transition-colors duration-300 ${isDarkMode ? 'bg-[#0f172a]/80 border-slate-800 backdrop-blur-md' : 'bg-white border-slate-100 shadow-sm'}`}>
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h1 className={`text-lg font-bold tracking-tight ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>Admission Queue</h1>
                            <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Global Fleet Expansion Registry</p>
                        </div>
                    </div>
                    ...
                    <div className="max-w-7xl mx-auto space-y-8 pb-12">
                        ...
                        <div className={`p-8 rounded-[2.5rem] border relative overflow-hidden transition-all duration-300 ${isDarkMode ? 'bg-[#1e293b]/50 border-slate-800' : 'bg-white border-slate-100 shadow-sm'}`}>
                            <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
                                <div>
                                    <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest mb-4 ${isDarkMode ? 'bg-indigo-500/10 text-indigo-400' : 'bg-indigo-50 text-indigo-600'}`}>
                                        <ClipboardList className="w-3 h-3" /> Fleet Protocol
                                    </div>
                                    <h2 className={`text-3xl font-black tracking-tight mb-2 ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>
                                ...
                                <div className={`px-8 py-4 rounded-3xl border text-center transition-all ${isDarkMode ? 'bg-slate-900/50 border-slate-800' : 'bg-slate-50 border-slate-100'}`}>
                                    <p className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 mb-1">Queue Density</p>
                                    <p className={`text-4xl font-black ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>{agents.length}</p>
                                </div>
                            </div>
                        </div>
                                ...
                                {isLoading ? (
                                    ...
                                    <motion.div
                                        ...
                                        className={`group rounded-[2.5rem] border p-6 transition-all duration-300 hover:translate-y-[-4px] ${isDarkMode
                                            ? 'bg-[#1e293b]/50 border-slate-800 hover:border-indigo-500/50 hover:bg-indigo-500/5 shadow-2xl shadow-indigo-500/5'
                                            : 'bg-white border-slate-100 shadow-sm hover:shadow-xl hover:shadow-slate-200/50'
                                            }`}
                                    >
                                        <div className="flex items-start justify-between mb-6">
                                            <div className={`w-14 h-14 rounded-2xl flex items-center justify-center border transition-all ${isDarkMode ? 'bg-slate-900 border-slate-800 text-indigo-400' : 'bg-slate-50 border-slate-100 text-indigo-600'
                                                }`}>
                                        ...
                                        <div className="space-y-4">
                                            <div>
                                                <h3 className={`text-lg font-black truncate ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>{agent.user_email}</h3>
                                            ...
                                            <div className={`grid grid-cols-2 gap-3 p-4 rounded-3xl border ${isDarkMode ? 'bg-slate-900/50 border-slate-800' : 'bg-slate-50 border-transparent shadow-inner'}`}>
                                                <div>
                                                    <p className="text-[9px] font-black uppercase text-slate-500 mb-1">Vehicle Unit</p>
                                                    <p className={`text-xs font-black uppercase tracking-tight ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>{agent.vehicle_type}</p>
                                                </div>
                                                <div>
                                                    <p className="text-[9px] font-black uppercase text-slate-500 mb-1">Registry Date</p>
                                            ...
                                            <button
                                                ...
                                                className={`w-full py-4 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] flex items-center justify-center gap-2 transition-all ${isDarkMode
