                ...
                <div className={`flex items-center justify-between p-6 border-b transition-colors duration-300 ${isDarkMode ? 'border-slate-800' : 'border-slate-100'}`}>
                    <div className="flex items-center gap-0 group cursor-pointer" onClick={() => navigate('/dashboard')}>
                        ...
                        <div className="flex flex-col -ml-5">
                            ...
                            <span className="text-[9px] font-black uppercase tracking-[0.4em] mt-0.5 text-slate-500">Admin Portal</span>
                        </div>
                    </div>
                </div>
                        ...
                        return (
                            <button
                                ...
                                className={`flex items-center gap-3 px-4 py-3 text-sm font-semibold rounded-xl transition-all duration-200 group w-full text-left ${isActive
                                    ...
                                    : isDarkMode ? 'text-slate-400 hover:bg-slate-800/50 hover:text-white' : 'text-slate-500 hover:bg-slate-50 hover:text-indigo-600'
                                    } ${item.isPlaceholder ? 'cursor-default' : ''}`}
                            >
                                <item.icon className={`w-4 h-4 transition-transform group-hover:scale-110 ${isActive ? 'text-white' : isDarkMode ? 'text-slate-500 group-hover:text-indigo-400' : 'text-slate-400 group-hover:text-indigo-600'}`} />
                ...
                <div className="p-4 mt-auto space-y-3">
                    ...
                    <button
                        ...
                        className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl border transition-all duration-300 group ${isDarkMode
                            ...
                            : 'bg-slate-50 border-slate-100 text-indigo-600 hover:bg-white hover:border-indigo-200 shadow-sm'
                            }`}
                    >
                        {isDarkMode ? (
                            ...
                            <div className="flex items-center gap-3 w-full">
                                ...
                                <span className="text-xs font-bold uppercase tracking-wider text-slate-600">Dark Mode</span>
                            </div>
                        )}
                    </button>
                    ...
                    <div className={`flex items-center gap-3 p-3 rounded-xl border transition-colors duration-300 ${isDarkMode ? 'bg-slate-800/50 border-slate-700' : 'bg-slate-50 border-slate-100'}`}>
                        <div className="relative group">
                            <div className={`absolute inset-0 blur-md opacity-20 bg-indigo-600 rounded-full transition-opacity group-hover:opacity-40`}></div>
                            <div className="w-10 h-10 rounded-full bg-gradient-to-br from-indigo-500 to-violet-600 flex items-center justify-center text-white font-bold shadow-lg shadow-indigo-500/20 relative z-10">
                                A
                            </div>
                        </div>
                        <div className="flex-1 min-w-0">
                            <p className={`text-xs font-black truncate transition-colors duration-300 ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>ADMIN USER</p>
